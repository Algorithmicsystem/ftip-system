...
def compute_superfactor(
    ...
    aligned = pd.concat(
        [alpha_series, sentiment, crowd_accel, regime_score, momentum], axis=1
    ).fillna(0.0)
...
class SuperfactorModel:
    ...
    def fit_transform(self, features: pd.DataFrame) -> pd.Series:
        ...
        sentiment = features.get(
            "sentiment_score", pd.Series(0.0, index=features.index)
        )
        crowd_accel = features.get("crowd_accel", pd.Series(0.0, index=features.index))
        regime_score = features.get(
            "regime_score", pd.Series(0.0, index=features.index)
        )
        momentum = features.get("mom_5", pd.Series(0.0, index=features.index))
        return compute_superfactor(
            structural_alpha, sentiment, crowd_accel, regime_score, momentum
        )
