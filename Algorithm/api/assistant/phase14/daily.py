...
def _changed_signals(current_report: Dict[str, Any], prior_report: Dict[str, Any]) -> List[str]:
    if not prior_report:
        return ["No prior same-symbol report is available, so this review is establishing the baseline shadow record."]
    changes: List[str] = []
    ...
    if current_signal != prior_signal:
        changes.append(f"Signal changed from {prior_signal or 'n/a'} to {current_signal or 'n/a'}.")
    ...
    if current_posture != prior_posture:
        changes.append(
            f"Strategy posture changed from {prior_posture or 'n/a'} to {current_posture or 'n/a'}."
        )

    if current_report.get("deployment_permission") != prior_report.get("deployment_permission"):
        changes.append(
            f"Deployment permission moved from {prior_report.get('deployment_permission') or 'n/a'} to {current_report.get('deployment_permission') or 'n/a'}."
        )
    if current_report.get("trust_tier") != prior_report.get("trust_tier"):
        changes.append(
            f"Trust tier shifted from {prior_report.get('trust_tier') or 'n/a'} to {current_report.get('trust_tier') or 'n/a'}."
        )
    if current_report.get("current_operating_mode") != prior_report.get("current_operating_mode"):
        changes.append(
            f"Operating mode moved from {prior_report.get('current_operating_mode') or 'n/a'} to {current_report.get('current_operating_mode') or 'n/a'}."
        )
    if current_report.get("candidate_classification") != prior_report.get("candidate_classification"):
        changes.append(
            f"Candidate classification moved from {prior_report.get('candidate_classification') or 'n/a'} to {current_report.get('candidate_classification') or 'n/a'}."
        )
    if not changes:
        changes.append("No material change was detected versus the prior same-symbol report.")
    return changes
...
def build_daily_workflow(
    ...
    *,
    ...
    what_changed = " ".join(changed_signals[:3])
    daily_summary = (
