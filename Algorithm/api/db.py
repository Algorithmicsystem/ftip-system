...
def upsert_universe(symbols: Iterable[str], source: Optional[str] = None) -> tuple[int, int]:
    """Upsert up to 1000 symbols into prosperity_universe.

    Returns a tuple of (received_count, upserted_count).
    """

    normalized = _normalize_symbols(symbols)
    if len(normalized) > 1000:
        raise ValueError("maximum of 1000 symbols allowed")

    # Avoid duplicate upserts within the same batch
    ...
    with pool.connection(timeout=10) as conn:
        with conn.cursor() as cur:
            cur.execute("SET LOCAL statement_timeout TO %s", (5000,))
            params = [(sym, True, source) for sym in unique_symbols]
            cur.executemany(
                """
    ...
    return len(normalized), len(unique_symbols)
