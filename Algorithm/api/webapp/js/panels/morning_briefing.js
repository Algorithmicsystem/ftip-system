...
function renderBriefing(data) {
  ...
  const sriCls = sri >= 70 ? 'warning' : sri >= 40 ? 'elevated' : 'stable';
  ...
  const icState = (data.ml_model_health?.recommendation || 'unknown').toUpperCase();
  ...
  body.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
      ...
      <div class="metric-card">
        ...
        <span class="metric-card__value" style="color:${sri>=70?'var(--signal-sell)':sri>=40?'var(--signal-hold)':'var(--signal-buy)'}">${sri.toFixed(1)}</span>
      ...
      <!-- Cross Asset -->
      <div class="metric-card">
        <span class="metric-card__label">Cross-Asset Confirmation</span>
        <span class="metric-card__value">${(ca.cross_asset_confirmation_score ?? 0).toFixed(0)}</span>
