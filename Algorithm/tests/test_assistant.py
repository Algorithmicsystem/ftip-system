...
def test_generate_analysis_report_persists_artifact(monkeypatch):
    ...
    assert result["axiom_evidence_backed_deployability_tier"]
    assert result["axiom_portfolio_fit_label"]
    assert result["axiom_portfolio_rank_score"] is not None
    ...
    assert result["axiom_explanation"]["support_vs_drag_summary"]
    assert result["axiom_explanation"]["setup_character_summary"]
    assert result["axiom_explanation"]["false_positive_risk_summary"]
    assert result["axiom_explanation"]["decision_hierarchy_summary"]
    assert result["axiom_proprietary_synthesis"]
    assert result["axiom_support_vs_drag_summary"]
    assert result["axiom_why_now_summary"]
    assert result["axiom_unique_mispricing_summary"]
    assert result["axiom_setup_character_summary"]
    assert result["axiom_false_positive_risk_summary"]
    assert result["axiom_decision_hierarchy_summary"]
    assert result["axiom_exceptionality_summary"]
    assert result["data_provider_quality_summary"]
    ...
    assert (
        store.get_latest_artifact(
            kind=AXIOM_SCORE_HISTORY_ARTIFACT_KIND, session_id=result["session_id"]
        )
        is not None
    )
    assert (
        store.get_latest_artifact(
            kind=AXIOM_CALIBRATION_ARTIFACT_KIND, session_id=result["session_id"]
        )
        is not None
    )
    assert (
