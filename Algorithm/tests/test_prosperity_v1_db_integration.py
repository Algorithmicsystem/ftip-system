...
class TestProsperityV1DBIntegration:
    ...
    def test_v1_round_trip_snapshot_latest_endpoints_with_real_db(
        ...
        monkeypatch.setattr(
            "api.main.massive_fetch_daily_bars",
            fake_massive_fetch_daily_bars,
        )
        ...
        with TestClient(app) as client:
            ...
            assert bootstrap_body["status"] == "ok"
            assert bootstrap_body["db_enabled"] is True
            ...
            assert snapshot_body["status"] == "ok"
            assert snapshot_body["result"]["rows_written"]["signals"] == 1
            assert snapshot_body["result"]["rows_written"]["features"] == 1
            assert snapshot_body["result"]["symbols_ok"] == [symbol]
            ...
            assert signal_body["lookback"] == lookback
            assert signal_body["as_of"] == as_of.isoformat()
            assert signal_body["signal"] in {"BUY", "SELL", "HOLD"}
            ...
            assert features_body["lookback"] == lookback
            assert features_body["as_of"] == as_of.isoformat()
            assert isinstance(features_body["features"], dict)
    ...
    def test_v1_daily_snapshot_job_writes_and_reads_back(
        ...
        with TestClient(app) as client:
            ...
            job_body = job.json()
            assert job_body["status"] in {"ok", "partial"}
            assert symbol in job_body["symbols_ok"]

            as_of = dt.date.fromisoformat(job_body["as_of_date"])
            ...
            assert latest_signal.status_code == 200
            assert latest_signal.json()["symbol"] == symbol
            ...
            assert latest_features.status_code == 200
            assert latest_features.json()["symbol"] == symbol
